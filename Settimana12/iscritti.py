...
def estrai_cds_con_insiemi(studenti):
    elenco = set()
    for studente in studenti:
        cds = studente[COLONNA_CDS]
        elenco.add(cds)
...
def conta_frequenze(studenti):
    """
    calcola per ogni CdS il numero di studenti frequentanti

    Args:
        studenti (list): elenco degli studenti

    Returns:
        dict -- costruisce un dizionario così composto:
            chiave: stringa (nome CdS)
            valore: numero intero (frequenza di apparizione)
    """
    ...
    for studente in studenti:
        cds = studente[COLONNA_CDS]
        if cds in frequenza:
            frequenza[cds] = frequenza[cds] + 1
        else:
            frequenza[cds] = 1

        # frequenza[cds] = frequenza.get(cds, 0) + 1
    print(frequenza)
    return frequenza
